...
namespace LearningApp.Service.Database.Repositories
{
	public interface IDbRepository
	{
		...
		IEnumerable<T> Get<T>(Expression<Func<T, bool>> selector, bool noTracking = false) where T : class, ITable;
		T Get<T>(long id, bool noTracking = false) where T : class, ITable;
		IEnumerable<T> GetAll<T>(bool noTracking = false) where T : class, ITable;
